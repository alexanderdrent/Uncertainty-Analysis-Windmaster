...
from ema_workbench.em_framework.evaluators import MultiprocessingEvaluator, MC, SequentialEvaluator
...
class WindMasterModel(netlogo.NetLogoModel):
    ...
    def run_experiment(self, experiment):
        ...
        results = {'stedin_capacity': [entry[0] for entry in stedin],
                   ...
                   'investments': investments.sum(axis=0),
                   'first_failure': year,
                   'h2_import': h2_import,
...
if __name__ == '__main__':
    ...
    with open('delta_dicts_list.json') as f:
    ...
    for assetid in assets[assettype.name]:
        uncertainties += [BooleanParameter(f"baseload biomass or closure {assetid}"),
                          IntegerParameter(f"{assetid} year", 2020, 2030)]

    uncertainties += [IntegerParameter("yearGunvorPhaseout", 2025, 2030),
                      IntegerParameter("yearKochPhaseout", 2025, 2030),
                      IntegerParameter("yearBPOffline", 2035, 2040),
                      BooleanParameter("yearHydrocrackerBP"),
                      IntegerParameter("year of introduction ELECTROLYSIS_H20", 2028, 2050),
                      # tov earliest H2 availability
                      CategoricalParameter("location h2 production",
                                           ["maasvlakte", "botlek", "none"]),  # asset ids
                      IntegerParameter("timing CCS", 2022, 2030),
                      CategoricalParameter("SF factory 2032 location",
                                           ["maasvlakte", "botlek", "none"]),  # synthetic fuel --> events lijn 168/169
                      CategoricalParameter("SF factory 2036 location",
                                           ["maasvlakte", "botlek", "none"]),
                      CategoricalParameter("SF factory 2040 location",
                                           ["maasvlakte", "botlek", "none"]),
                      CategoricalParameter("SF factory 2044 location",
                                           ["maasvlakte", "botlek", "none"]),
                      CategoricalParameter("SF factory 2048 location",
                                           ["maasvlakte", "botlek", "none"]),
                      BooleanParameter("offshore wind growth"),
                      IntegerParameter("easterly demand H2", 30, 172),
                      IntegerParameter("year end demand gas Roterdam", 2030, 2045),
                      CategoricalParameter("decisionMakingModel",
                                           ["Reactive", "Current",
                                            "Proactive", "Collaborative"]),
                      RealParameter("capex_factor", 0.8, 1.2),
                      RealParameter("leadtime_factor", 0.8, 1.2),
